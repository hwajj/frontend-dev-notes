...
## 📘 **문제 5: 프로그래머스 Lv1. 최소직사각형**

> 여러 명함 `[w, h]`를 회전해 넣을 수 있는 가장 작은 지갑의 넓이를 구한다.
...
```js
// 최소직사각형
// 모든 명함을 [긴 변, 짧은 변]으로 통일한 뒤 max(긴 변) × max(짧은 변)
// (예시 문제)
// - 프로그래머스 Lv1. 최소직사각형
...
- 회전이 자유로우므로 각 명함의 가로·세로 중 **긴 쪽을 가로, 짧은 쪽을 세로**로 맞춘다.
- 모든 명함을 수납하려면 지갑 가로 ≥ 모든 긴 변의 최댓값, 세로 ≥ 모든 짧은 변의 최댓값이어야 한다.
- 그리디 관점: 각 카드를 같은 방향으로 정규화하는 "지금 당장의 선택"이 전체 최소 넓이로 이어진다.
